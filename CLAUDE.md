...
**`Route.sourceUrl` (added independently, reconciled 2026-09-10):** a route can carry a `sourceUrl` pointing straight at a live official search/query page (e.g. Copperas Cove's own site search, a Municode ordinance-library search, Gatesville's own agendas page). For the `meetings` tool, arriving via a `sourceUrl` route redirects the resident there immediately (`window.location.assign`) — untouched by the Layer 2 work above. For the `documents` tool, the curated index is checked **first**; `sourceUrl` only surfaces as a labeled fallback link when the jurisdiction's index has no match. Never let `sourceUrl` short-circuit a documents-tool query before the index has had a chance to answer.
